...
class XtenderCustomCode{
  ...
  $options[] = array(
  	...
  	'id'   		=> 'code_head',
  	'section'	=> 'section_code',
  	'desc'		=> esc_html__( 'Please use valid HTML5 rules only.', 'xtender' ),
  ...
  $options[] = array(
  	...
  	'id'   		=> 'section_css',
  	'panel'		=> 'panel_dev',
  	'desc'		=> esc_html__( 'Please use valid CSS3 rules only.', 'xtender')
  );
  $options[] = array(
  ...
  public static function custom_head_code() {
        $custom_head = get_theme_mod( 'code_head' );
		echo filter_var( esc_attr( get_theme_mod( 'code' ) ), FILTER_VALIDATE_BOOLEAN ) && isset( $custom_head ) && $custom_head !== false && ! empty( $custom_head ) ?  htmlspecialchars_decode( $custom_head ) : '';
	}
