	...
	class xtenderVCIsotope{
		...
		function load_assets(){
			...
			if( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'curly_isotope') ) {
				...
				wp_enqueue_script( 'xtender-isotope', XTENDER_URL . 'assets/front/js/xtender-owl-carousel-min.js', array( 'jquery','jquery-migrate','isotope', 'imagesloaded' ), rand(), true );
			}
		...
		public function isotope_item( $atts, $content = null ){
			...
			$link = $link !== '#' && strlen($link) > 2 ? esc_url_raw( $link ) : '#isotope-'.$_SESSION['curly-isotope'];
